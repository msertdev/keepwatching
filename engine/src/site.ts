...
export function buildSite(opts: BuildOptions = {}): Gallery {
  ...
  fs.writeFileSync(path.join(SITE_DIR, "gallery.json"), json, "utf8");

  /* The same data as a plain script, so the page also works when it is opened
     straight off disk. `fetch` is blocked on the file: scheme; a <script> tag
     is not — and double-clicking index.html is what people actually do. */
  fs.writeFileSync(
    path.join(SITE_DIR, "gallery.js"),
    "/* Generated by `kw gallery`. Lets the page load over file://, where fetch is blocked. */\n" +
  ...
  const missing = cards.filter((c) => !c.media.mp4 && !c.media.webm);
  ...
  /* A gallery of previews, built without previews, is not a success. Say what
     is wrong and exit non-zero rather than printing a hint and returning 0 —
     looking like it worked is the failure this repo exists to prevent. */
  if (missing.length && !opts.allowMissing) {
    const names = missing.slice(0, 6).map((m) => m.slug).join(", ");
    console.error(
      `\n\x1b[31m✗ gallery is not usable: ${missing.length} of ${cards.length} formats have ` +
        `no rendered preview.\x1b[0m`
    );
    console.error(`  missing: ${names}${missing.length > 6 ? `, +${missing.length - 6} more` : ""}`);
    console.error(`\n  Render them and rebuild in one step:\n`);
    console.error(`      npx kw gallery\n`);
    console.error(`  Or build anyway, accepting cards with empty frames:\n`);
    console.error(`      npx kw site build --allow-missing\n`);
    process.exit(1);
  }
